module overmind::nftango {
    ...
    public fun assert_nftango_store_has_an_opponent(
        ...
        let store = borrow_global<NFTangoStore>(account_address);
        assert!(!option::is_none<address>(&store.opponent_address), ERROR_NFTANGO_STORE_IS_ACTIVE);
    ...
    public fun assert_nftango_store_does_not_have_an_opponent(
        ...
        let store = borrow_global<NFTangoStore>(account_address);
        assert!(option::is_none<address>(&store.opponent_address), ERROR_NFTANGO_STORE_DOES_NOT_HAVE_AN_OPPONENT);
